...
NEWS_DIR = BASE_DIR  # BASE_DIR 就是 secretary-team 目录
...
def collect_today_reports():
    ...
    # 新闻报告目录（注意：AI News实际输出到news-daily目录）
    report_dirs = {
        "AI News": os.path.join(NEWS_DIR, "news-daily", "output", today),
        "Pharma News": os.path.join(NEWS_DIR, "pharma-news", "output", today),
        "Medical Device News": os.path.join(NEWS_DIR, "medical-device-news", "output", today),
    }
...
def main():
    ...
    # 2. 运行新闻助理
    scripts = [
        ("AI News Agent", os.path.join(NEWS_DIR, "ai-news", "server.py"), "ai_news.html", "AI News Daily"),
        ("Pharma News Agent", os.path.join(NEWS_DIR, "pharma-news", "server.py"), "pharma_news.html", "医药生物制药新闻"),
        ("Medical Device News Agent", os.path.join(NEWS_DIR, "medical-device-news", "server.py"), "medical_news.html", "医疗器械新闻"),
    ]
    ...
    for name, path, html_file, subject in scripts:
        ...
        # 如果邮件发送失败，尝试重试
        if not email_sent:
            # 确定正确的HTML路径（AI News实际输出到news-daily目录）
            if "AI News" in name:
                # AI News 实际输出到 news-daily 目录
                html_path = os.path.join(NEWS_DIR, "news-daily", "output", today, html_file)
            elif "Pharma" in name:
                html_path = os.path.join(NEWS_DIR, "pharma-news", "output", today, html_file)
            elif "Medical" in name:
                html_path = os.path.join(NEWS_DIR, "medical-device-news", "output", today, html_file)
            else:
                html_path = None
